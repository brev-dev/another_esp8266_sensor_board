...
Combining an esp8266 with a LORA module is a stretch when it comes to available gpio. This board is designed to work with SX1276 LORA modules ([example link](https://www.aliexpress.com/item/32984655636.html)), and leans heavily on the design from [here](https://github.com/hallard/WeMos-Lora), which minimizes needed ports by linking three of the LORA module's pins (DIO0, 1 and 2) to one esp8266 pin via three diodes (board footprints D9, D10 and D11 respectively). This DIO-sharing configuration needs special handling in software. However, for simple LORA communications only DIO0 is needed, so for my LORA boards to date I have just bridged D9, and left D10 and D11 disconnected.
